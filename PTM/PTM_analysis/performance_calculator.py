...
class PredictedToImpactPerformanceAnalyzer:
    def __init__(self, DataDir, RPyr, leadtime, impactData, triggerProb, adminLevel, adminPath, startYear, endYear, years, PredictedEvents_gdf, comparisonType, actionLifetime, model):
        """
    ...
    def openModel_events (self, predictedEvents_gdf): 
        ...
        # Merge the CSV data with the shapefile data
        modelevents= pd.merge(self.gdf_shape, df_filtered, how='left', left_on=f'ADM{cfg.adminLevel}', right_on=f'ADM{cfg.adminLevel}')
    ...
    def calculateCommunePerformance(self):
        ...
        scores_byCommune_gdf.to_file (f"{self.DataDir}/{self.model}/{self.comparisonType}/scores_byCommuneRP{self.RPyr:.1f}_yr_leadtime{self.leadtime:.0f}.gpkg")
        scores_byCommune_gdf.drop(columns='geometry').to_csv (f"{self.DataDir}/{self.model}/{self.comparisonType}/scores_byCommuneRP{self.RPyr:.1f}_yr_leadtime{self.leadtime:.0f}.csv")
...
if __name__=='__main__':
    ...
    for RPyr in cfg.RPsyr: 
        ...
        for leadtime in cfg.leadtimes: 
            ...
            analyzer = PredictedToImpactPerformanceAnalyzer(cfg.DataDir, RPyr, leadtime, impact_csv, cfg.triggerProb, cfg.adminLevel, cfg.admPath, cfg.startYear, cfg.endYear, cfg.years, PTM_events, comparisonType, cfg.actionLifetime, 'PTM')

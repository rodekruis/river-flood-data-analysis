...
class PredictedToImpactPerformanceAnalyzer:
    ...
    def openObservedImpact_gdf(self, typeImpact):
        ...
        if isinstance(self.impactData, str): 
            if self.impactData.endswith('.csv'):
                delimiter = ";" if typeImpact == "Observed" else ","
    ...
    def openModel_events (self, predictedEvents_gdf): 
        ...
        try:
            ...
            print(f'No model prediction data for leadtime: {self.leadtime}, RP: {self.RPyr}')
...
if __name__=='__main__':
    ...
    percentile = 1 # percentile to compare to for observational data
    #observation_stations = loop_over_stations(cfg.DNHstations, cfg.DataDir, percentile)
    #observation_per_adm = events_per_adm(cfg.DataDir, cfg.admPath, cfg.adminLevel, cfg.DNHstations, cfg.stationsDir, observation_stations, 'Observation', percentile)
    observation_per_adm = pd.read_csv (f"{cfg.DataDir}/Observation/floodevents_admUnit_RP1yr.csv")
    for RPyr in cfg.RPsyr: 
         # okay so now return period, setting it low to a standard to make sure there are many hits 
        # f'{cfg.DataDir}/{comparisonType}/observationalStation_flood_events_RP_{RPyr:.1f}yr.csv'
                    # PTM_events = f'{cfg.DataDir}/PTM/floodevents_admUnit_RP{RPyr}yr.csv'
        ...
        for leadtime in cfg.leadtimes:
            ...
            # analyzer.matchImpact_and_Trigger()
            # analyzer.calculateCommunePerformance()
            glofas_event_gpkg = f"{cfg.DataDir}/GloFAS/floodedRP{RPyr:.1f}yr_leadtime{leadtime}_ADM2.gpkg"
            floodProbability_gdf = checkVectorFormat (glofas_event_gpkg)
            if floodProbability_gdf.empty:
                continue
            else:
                flooddefiner = FloodDefiner (cfg.adminLevel)
                glofas_event_gdf =  flooddefiner.EventMaker(floodProbability_gdf, cfg.actionLifetime, cfg.triggerProb)
                analyzerG = PredictedToImpactPerformanceAnalyzer(cfg.DataDir, 
                                                                    RPyr, 
                                                                    observation_per_adm, 
                                                                    cfg.triggerProb, 
                                                                    cfg.adminLevel, 
